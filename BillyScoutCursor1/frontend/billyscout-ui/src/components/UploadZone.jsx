...
function UploadZone() {
  ...
  const validateUrl = (url) => {
    try {
      ...
      // Check if it's a supported broadcast platform
  ...
  const handleUrlSubmit = async (e) => {
    ...
    if (!validateUrl(videoUrl)) {
      setError('Please enter a valid URL from a supported broadcast platform');
      return;
    }
        ...
        <form onSubmit={handleUrlSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <button
                type="button"
                onClick={() => navigator.clipboard.readText().then(setVideoUrl)}
                className="absolute right-2 top-1/2 transform -translate-y-1/2 px-3 py-1 text-sm text-blue-500 hover:text-blue-600"
              >
                Paste
              </button>
            </div>
            <p className="mt-1 text-sm text-gray-500">
              Paste the URL of the broadcast video from North Coast Network, Hudl, YouTube, or Vimeo
            </p>
          </div>
          <button
            type="submit"
